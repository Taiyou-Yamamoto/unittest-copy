...
export const PostForm = (props: Props) => {
  ...
  return (
    <form aria-label={props.title} className={styles.module} onSubmit={handleSubmit(props.onValid, props.onInvalid)}>
      <div className={styles.content}>
        <div className={styles.meta}>
          <PostFormInfo register={register} control={control} errors={errors} />
          {/* <PostFormHeroImage register={register} setValue={setValue} name='imageUrl' error={errors.imageUrl?.message} /> */}
        </div>
        <TextareaWithInfo {...register('body')} title='本文' rows={5} error={errors.body?.message} />
      </div>
      <PostFormFooter isSubmitting={isSubmitting} register={register} control={control} onClickSave={props.onClickSave} />
      {props.children}
    </form>
  );
};
