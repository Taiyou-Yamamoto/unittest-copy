...
function setup() {
  ...
  render(<PostForm title='新規記事' onClickSave={onClickSave} onValid={onValid} onInvalid={onInvalid} />);
...
test('不適正内容で「下書き保存」を試みると、バリデーションエラーが表示される', async () => {
  ...
  await saveAsDraft();
  await waitFor(() => expect(screen.getByRole('textbox', { name: '記事タイトル' })).toHaveErrorMessage('1文字以上入力してください'));
  //冗長なので統一
...
test('適正内容で「記事を公開」を試みると、onClickSave イベントハンドラーのみ実行される', async () => {
  const { typeTitle, saveAsPublished, onClickSave, onValid, onInvalid } = setup();
  await typeTitle('私の技術記事');
